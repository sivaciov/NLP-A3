...
def predict_two_classes(examples: List[FactExample], fact_checker):
    ...
    for i, example in enumerate(tqdm(examples)):
        ...
        # if pred_label != gold_label then print the example and passages
        if pred_label != gold_label:
            # check if this is false positive or false negative
            str_pos_neg = "false positive" if pred_label == 0 else "false negative"
            print(f"Example {i}: FP/FN: {str_pos_neg}")
